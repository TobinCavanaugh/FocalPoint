...
Shader "PPU/ChromaticAberration" {
	...
	HLSLINCLUDE
			...
			half3 Frag0 (VaryingsDefault i) : SV_Target {
				...
				CameraOutput.x = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, half2((i.texcoordStereo.x - (_Offset / 30.0)),i.texcoordStereo.y)).x;
				CameraOutput.y = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoordStereo).y;
				CameraOutput.z = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, half2((i.texcoordStereo.x + (_Offset / 30.0)),i.texcoordStereo.y)).z;
				return CameraOutput;
			}
