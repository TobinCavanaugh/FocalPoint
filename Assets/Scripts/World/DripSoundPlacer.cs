...
namespace World
{
    public class DripSoundPlacer : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            //Raycast down
            if (Physics.Raycast(audioSource.transform.position - new Vector3(0, 1, 0), Vector3.down, out var hit, 99f,
                    _lm))
            {
        ...
        {
            if (_doShake &&
                Vector3.Distance(audioSource.transform.position, Shaker.GlobalShakers[0].transform.position) <=
                _minShakeDist)
